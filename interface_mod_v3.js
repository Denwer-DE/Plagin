(function () {
  ...
  var OnlineIcons = {
    ...
    patchButton: function (btn) {
      ...
      var titles = { bwarc: 'BwaRC', dso: 'DSO', lampac: 'Lampac' };
    ...
    enable: function () {
      ...
      // Setup observer
      Utils.safeDisconnect(State.observers.online);
      State.observers.online = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.patch_online_icons) return;
        mutations.forEach(function (m) {
          if (m.addedNodes && m.addedNodes.length) {
            for (var i = 0; i < m.addedNodes.length; i++) {
              OnlineIcons.patchButtonsInRoot(m.addedNodes[i]);
            }
          }
        });
      });
      State.observers.online.observe(document.body, { childList: true, subtree: true });
    },

    disable: function () {
      Utils.safeDisconnect(State.observers.online);
      delete State.observers.online;
  ...
  var CardEnhancer = {
    ...
    enable: function () {
      ...
      // Setup observer for new cards and icon changes
      Utils.safeDisconnect(State.observers.cardEnhancer);
      State.observers.cardEnhancer = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.use_custom_styles) return;
        mutations.forEach(function (m) {
          // Handle added nodes (new cards)
          if (m.addedNodes && m.addedNodes.length) {
            for (var i = 0; i < m.addedNodes.length; i++) {
              var node = m.addedNodes[i];
              if (!node || !node.classList) continue;
              if (node.classList.contains('card')) {
                CardEnhancer.processCard(node);
              } else if (node.classList.contains('card__icon')) {
                // Icon added - update parent card's visibility
                var $card = $(node).closest('.card');
                if ($card.length) CardEnhancer.updateIconsVisibility($card);
              } else if (node.querySelectorAll) {
                var cards = node.querySelectorAll('.card');
                for (var j = 0; j < cards.length; j++) {
                  CardEnhancer.processCard(cards[j]);
                }
              }
            }
          }
          // Handle removed nodes (icons removed)
          if (m.removedNodes && m.removedNodes.length) {
            for (var k = 0; k < m.removedNodes.length; k++) {
              var removed = m.removedNodes[k];
              if (removed && removed.classList && removed.classList.contains('card__icon')) {
                // Icon removed - update the card that contained it
                var $parentCard = $(m.target).closest('.card');
                if ($parentCard.length) CardEnhancer.updateIconsVisibility($parentCard);
              }
            }
          }
        });
      });
      State.observers.cardEnhancer.observe(document.body, { childList: true, subtree: true });
    },

    disable: function () {
      Utils.safeDisconnect(State.observers.cardEnhancer);
      delete State.observers.cardEnhancer;
  ...
  var MovieTypeLabels = {
    enable: function () {
      ...
      // Setup observer
      Utils.safeDisconnect(State.observers.movieType);
      State.observers.movieType = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.show_movie_type) return;
        mutations.forEach(function (m) {
          if (m.addedNodes && m.addedNodes.length) {
            for (var i = 0; i < m.addedNodes.length; i++) {
              ...
              if (!node || !node.classList) continue;
              if (node.classList.contains('card')) {
                MovieTypeLabels.addToCard(node);
              } else {
                $(node).find('.card').each(function () {
                  MovieTypeLabels.addToCard(this);
                });
              }
            }
          }
      ...
      State.observers.movieType.observe(document.body, { childList: true, subtree: true });
    },
    ...
    disable: function () {
      Utils.safeDisconnect(State.observers.movieType);
      delete State.observers.movieType;
  ...
  var ColoredRatings = {
    ...
    enable: function () {
      ...
      Utils.safeDisconnect(State.observers.ratings);
      State.observers.ratings = new MutationObserver(Utils.throttle(ColoredRatings.update, 100));
      State.observers.ratings.observe(document.body, { childList: true, subtree: true });
    },

    disable: function () {
      Utils.safeDisconnect(State.observers.ratings);
      delete State.observers.ratings;
      $(".card__vote, .full-start__rate, .full-start-new__rate, .info__rate, .card__imdb-rate, .card__kinopoisk-rate").css('color', '');
    }
  };
  ...
  var ColoredElements = {
    ...
    enable: function () {
      ...
      Utils.safeDisconnect(State.observers.elements);
      State.observers.elements = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.colored_elements) return;
        mutations.forEach(function (m) {
          if (m.addedNodes && m.addedNodes.length) {
            for (var i = 0; i < m.addedNodes.length; i++) {
              var node = m.addedNodes[i];
              $(node).find('.full-start__status').each(function () { ColoredElements.applyStatus(this); });
              $(node).find('.full-start__pg').each(function () { ColoredElements.applyAge(this); });
            }
          }
        });
      });
      State.observers.elements.observe(document.body, { childList: true, subtree: true });
    },

    disable: function () {
      Utils.safeDisconnect(State.observers.elements);
      delete State.observers.elements;
  ...
  var TorrentStyles = {
    ...
    // Update all torrent item styles
    update: function () {
    ...
    enable: function () {
      ...
      // Setup observer
      Utils.safeDisconnect(State.observers.torrent);
      State.observers.torrent = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.torrent_styles) return;
        ...
        for (var i = 0; i < mutations.length; i++) {
          ...
          if (m.type === 'characterData' ||
            (m.type === 'childList' && m.target && m.target.classList &&
              (m.target.classList.contains('torrent-item__bitrate') ||
                m.target.classList.contains('torrent-item__seeds') ||
                m.target.classList.contains('torrent-item__grabs') ||
                m.target.classList.contains('torrent-item__size')))) {
            needsUpdate = true;
            break;
          }
        }
        ...
        if (needsUpdate) {
          ...
          State.intervals.torrentDebounce = setTimeout(TorrentStyles.update, 60);
        }
      });

      State.observers.torrent.observe(document.body, {
        childList: true,
        subtree: true,
        characterData: true
      });
    ...
    disable: function () {
      Utils.safeDisconnect(State.observers.torrent);
      delete State.observers.torrent;
      clearTimeout(State.intervals.torrentDebounce);
  ...
  var ButtonsManager = {
    ...
    enable: function () {
      ...
      Utils.safeDisconnect(State.observers.buttons);

      var debounceTimer = null;
      var pendingCards = new Set();

      State.observers.buttons = new MutationObserver(function (mutations) {
        if (!isEnabled() || !State.settings.show_online_buttons) return;
        ...
        for (var i = 0; i < mutations.length; i++) {
          ...
          if (m.addedNodes && m.addedNodes.length) {
            for (var j = 0; j < m.addedNodes.length; j++) {
              ...
              // Check if it's a source button being added
              if (node.classList &&
                (node.classList.contains('view--online') ||
                  node.classList.contains('view--torrent') ||
                  node.classList.contains('view--trailer') ||
                  node.classList.contains('button--priority') ||
              ...
              if (node.querySelector) {
                if (node.querySelector('.view--online, .view--torrent, .view--trailer, .full-start-new')) {
                  shouldProcess = true;
                }
              }
            }
          ...
          // Class changes on buttons (e.g., hide -> visible)
          if (m.type === 'attributes' && m.attributeName === 'class') {
            var target = m.target;
            if (target && target.classList &&
              (target.classList.contains('view--torrent') ||
                target.classList.contains('view--online') ||
                target.classList.contains('full-start__button'))) {
              shouldProcess = true;
            }
          }
        ...
        if (shouldProcess) {
          // Process immediately for responsive navigation
          // Use minimal debounce only to batch rapid mutations
          clearTimeout(debounceTimer);
          debounceTimer = setTimeout(function () {
            try {
              // Find active full card
              var active = Lampa.Activity.active();
              if (active && active.activity && active.activity.render) {
              ...
              Utils.log('ButtonsManager observer error:', e);
            }
          }, 5); // Minimal debounce - process almost immediately
        }
      });

      State.observers.buttons.observe(document.body, {
        childList: true,
        subtree: true,
        attributes: true,
    ...
    disable: function () {
      Utils.safeDisconnect(State.observers.buttons);
      delete State.observers.buttons;
  ...
  var Settings = {
    ...
    register: function () {
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: { name: 'Иконки Online-провайдеров', description: 'BwaRC / DSO / Lampac' },
        onChange: function (v) {
