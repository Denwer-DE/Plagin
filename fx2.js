...
}
{
  ...
  ;(function () {
    ...
    var dev_token =
      ...
      unic_id +
      "&user_dev_name=Lampa&user_dev_os=11&user_dev_vendor=FXAPI&user_dev_token="
    var modalopen = false
    var ping_auth
      ...
      if (!fxapi_token) {
        ...
        var modal = $(
          '<div><div class="broadcast__text">' +
            Lampa.Lang.translate("modal_text") +
            '</div><div class="broadcast__device selector" style="text-align: center; background-color: darkslategrey; color: white;">' +
            Lampa.Lang.translate("modal_wait") +
        ...
        ping_auth = setInterval(function () {
          network.silent(
            Lampa.Utils.addUrlComponent(api_url + "user_profile", dev_token + user_token),
            function (json) {
        ...
        network.quiet(
          Lampa.Utils.addUrlComponent(api_url + "token_request", dev_token),
          function (found) {
            if (found.status == "ok") {
        ...
        if (results.last_episode && results.last_episode.season) {
          ...
          while (s--) {
            filter_items.season.push(
              Lampa.Lang.translate("torrent_serial_season") + " " + (results.last_episode.season - s),
            )
          }
        }
          ...
          if (serial && episodes.length > items.length && !params.similars) {
            ...
            left.forEach(function (episode) {
              ...
              if (episode.still_path) {
                ...
                loader.remove()
                image.append(
                  '<div class="online-prestige__episode-number">' + ("0" + episode.episode_number).slice(-2) + "</div>",
                )
              }
    ...
    function startPlugin() {
      ...
      var button = '<div class="full-start__button selector view--online" data-subtitle="Filmix v'.concat(
        manifest.version,
      ...
      function checkToken(token) {
        ...
        network.silent(
          Lampa.Utils.addUrlComponent(api_url + "user_profile", dev_token + token),
          function (json) {
            if (json) {
              if (json.user_data) {
